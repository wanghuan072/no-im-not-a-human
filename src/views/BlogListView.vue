        ...
        <section class="section blog-list">
            <div class="container">
                ...
                <div v-else-if="error" class="error-state">
                    ...
                    <button @click="loadBlogs" class="btn btn-primary">{{ $t('BlogPage.list.error.retry') }}</button>
                </div>
                ...
                <div v-else-if="blogs.length > 0" class="blog-grid">
                    <article v-for="blog in blogs" :key="blog.id" class="blog-card"
                        ...
                        <div class="blog-image">
                            <img :src="blog.imageUrl" :alt="blog.imageAlt" loading="lazy" @error="handleImageError" />
                        </div>
                ...
                <!-- Pagination -->
                <div v-if="!loading && !error && blogs.length > 0 && pagination.totalPages > 1" class="pagination">
                    <button v-if="pagination.currentPage > 1" @click="goToPage(pagination.currentPage - 1)"
                    ...
                    <button v-if="pagination.hasMore" @click="goToPage(pagination.currentPage + 1)"
                        class="btn btn-secondary pagination-btn">
    ...
    setup() {
        ...
        const loadBlogs = async (page = 1) => {
            try {
                ...
                const result = await getBlogsList(currentLanguage.value, {
        ...
        const navigateToBlog = (addressBar) => {
            const path = currentLanguage.value === 'en' ? `/blog/${addressBar}` : `/${currentLanguage.value}/blog/${addressBar}`
            router.push(path)
        }
        ...
        const setupSEO = async () => {
            const seoData = await import('@/utils/blogUtils').then(module =>
                module.getBlogListSEO(currentLanguage.value)
            )
            setSEO(seoData, router.currentRoute.value.path, 'blog-list')
        }
